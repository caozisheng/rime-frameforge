...
pub const MESH_HEADER_BYTES: usize = 32;
...
pub fn rasterize_vignette_mesh(
    ...
    if optical_center.iter().any(|value| !(0.0..=1.0).contains(value)) {
        return Err(invalid("FixVignetteRadial optical center must be within [0, 1]"));
    }
...
/// `channel` is the CFA phase channel; `pixel` is the pixel-center
/// coordinate (`id + 0.5`); `entries_offset` is in `f32` units.
///
/// # Panics
///
...
pub fn mesh_gain(
    ...
    let position = [
        (pixel[0] / extent[0] - geometry.origin[0]) / geometry.spacing[0],
        (pixel[1] / extent[1] - geometry.origin[1]) / geometry.spacing[1],
    ];
    ...
    let column = position[0].clamp(0.0, last_column);
    let row = position[1].clamp(0.0, last_row);
    #[expect(
        clippy::cast_possible_truncation,
    ...
    let entry = |row_index: u32, col_index: u32| {
        entries[entries_offset
            + usize::try_from(
                row_index * geometry.points[1] * geometry.planes + col_index * geometry.planes + plane,
            )
...
pub fn mesh_geometry(
    ...
    if mesh.points[0] == 0 || mesh.points[1] == 0 || mesh.planes == 0 {
        return Err(invalid("gain map mesh must have at least one node and plane"));
    }
    if !mesh.spacing.iter().all(|value| value.is_finite() && *value > 0.0)
        || !mesh.origin.iter().all(|value| value.is_finite())
        || mesh.spacing.iter().any(|value| value.abs() > f64::from(f32::MAX))
        || mesh.origin.iter().any(|value| value.abs() > f64::from(f32::MAX))
    {
        return Err(invalid("gain map mesh geometry must be finite and f32-representable"));
    }
    if mesh.entries.len()
