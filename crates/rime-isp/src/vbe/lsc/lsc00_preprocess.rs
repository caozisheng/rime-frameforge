use super::lsc_common::{MESH_HEADER_BYTES, VIGNETTE_MESH_POINTS, mesh_geometry, rasterize_vignette_mesh};
use crate::operator::{
...
pub(crate) fn run(
    ...
    // Source order is preserved: GainMap opcodes first (byte-faithful
    // passthrough), then one rasterized mesh per FixVignetteRadial opcode.
    let mut meshes: Vec<GainMapParameters> = context.gain_maps.clone();
    ...
    let mesh_count =
        u32::try_from(meshes.len()).map_err(|_| OperatorError::Preprocess {
            module_id,
    ...
    let header_count = u32::try_from(headers.len() / MESH_HEADER_BYTES)
    ...
    let entry_count = u32::try_from(entries_bytes.len() / size_of::<f32>())
        .map_err(|_| OperatorError::Preprocess {
            module_id,
...
fn validate_mesh(mesh: &GainMapParameters, module_id: &'static str) -> Result<(), OperatorError> {
    ...
    let expected = usize::try_from(mesh.points[0])
        .ok()
        .and_then(|rows| usize::try_from(mesh.planes).ok().map(|planes| rows * planes))
        .and_then(|rows_planes| {
            usize::try_from(mesh.points[1]).ok().map(|cols| rows_planes * cols)
        })
        .unwrap_or(usize::MAX);
