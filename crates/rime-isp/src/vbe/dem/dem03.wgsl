...
fn shared_saturation_clip(rgb:vec3<f32>)->vec3<f32>{let peak=max(max(rgb.r,rgb.g),rgb.b);let scaled=rgb/peak;let clipped=select(scaled,vec3<f32>(1.0),min(min(scaled.r,scaled.g),scaled.b)>=0.5);return select(rgb,clipped,peak>1.0);}
@compute @workgroup_size(8,8)
fn demosaic_vng_main(@builtin(global_invocation_id) gid:vec3<u32>){let extent=textureDimensions(input_tex);if(gid.x>=extent.x||gid.y>=extent.y){return;}let p=vec2<i32>(gid.xy);let rgb=shared_saturation_clip(reconstruct(p,extent));textureStore(output_tex,p,vec4<f32>(rgb,1.0));}
