...
fn bilinear(
    ...
    let top = values[row][column] * (1.0 - column_fraction) + values[row][next_column] * column_fraction;
    let bottom = values[next_row][column] * (1.0 - column_fraction) + values[next_row][next_column] * column_fraction;
...
fn locate(knots: &[f64], value: f64) -> (usize, f64) {
    ...
    let index = knots.partition_point(|knot| *knot <= value).saturating_sub(1).min(knots.len() - 2);
    (index, (value - knots[index]) / (knots[index + 1] - knots[index]))
}
...
mod tests {
    ...
    fn lookup_interpolates_scene_brightness_and_log_iso() {
        let low = lookup_default(AhdIqInput { scene_brightness_ev: 0.0, iso: 100.0 }).expect("valid input");
        let high = lookup_default(AhdIqInput { scene_brightness_ev: 8.0, iso: 800.0 }).expect("valid input");
    ...
    fn lookup_clamps_axis_values() {
        let below = lookup_default(AhdIqInput { scene_brightness_ev: -100.0, iso: 1.0 }).expect("clamped input");
        let at_min = lookup_default(AhdIqInput { scene_brightness_ev: -4.0, iso: 100.0 }).expect("minimum input");
        assert_eq!(below, at_min);
    }

    #[test]
    fn rejects_non_positive_iso() {
        assert!(lookup_default(AhdIqInput { scene_brightness_ev: 4.0, iso: 0.0 }).is_err());
    }
}
