...
const packetProvider: StagedFramePacketProvider = {
  ...
  prepareConsumers: () => ({
    tintlessUniform: new Uint8Array(48),
    tintlessMesh: new Uint8Array(65 * 49 * 2 * 4),
    tintlessAudit: new Uint8Array(20),
...
describe('fused executor RAW upload layout', () => {
  it('uses padded source row stride when uploading RAW', () => {
    ...
    new NormalGpuExecutor(fake.gpu, new Uint16Array(8).buffer, 0, 1, padded, 'single', packetProvider);
