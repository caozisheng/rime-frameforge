...
describe('NormalGpuExecutor frame reuse', () => {
  it('uploads a same-extent frame without reallocating GPU resources', () => {
    ...
    expect(fake.counts()).toEqual({ textureCreates: 30, textureDestroys: 0, bufferCreates: 4, bufferDestroys: 0, rawUploads: 2 });
  });
  ...
  it('destroys frame-owned GPU resources on disposal', () => {
    ...
    expect(fake.counts()).toEqual({ textureCreates: 30, textureDestroys: 30, bufferCreates: 4, bufferDestroys: 4, rawUploads: 1 });
  });
});
