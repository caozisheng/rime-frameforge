...
interface DrcLevelResources {
  ...
  readonly statsHorizontal: GPUTexture;
  readonly statsVertical: GPUTexture;
...
export class WebDrcExecutor {
  ...
  public constructor(device: GPUDevice, raw: ArrayBuffer, rawByteOffset: number, descriptor: RawFrameDescriptor) {
    ...
    this.#pipelines = {
      drc_prefilter_main: pipeline('drc_prefilter_main'),
      pyramid_downsample_main: pipeline('pyramid_downsample_main'),
      pyramid_reconstruct_main: pipeline('pyramid_reconstruct_main'),
      guided_stats_horizontal_main: pipeline('guided_stats_horizontal_main'),
      guided_stats_vertical_main: pipeline('guided_stats_vertical_main'),
      guided_coefficients_main: pipeline('guided_coefficients_main'),
      guided_coefficients_horizontal_main: pipeline('guided_coefficients_horizontal_main'),
      guided_apply_vertical_main: pipeline('guided_apply_vertical_main'),
      drc_combine_global_main: pipeline('drc_combine_global_main'),
      drc_combine_local_main: pipeline('drc_combine_local_main'),
    };
    this.#levels = levelExtents(descriptor.width, descriptor.height).map(([width, height], index) => ({
      ...
      statsHorizontal: createTexture(device, `drc-web-stats-h-${index}`, 'rgba32float', width, height),
      statsVertical: createTexture(device, `drc-web-stats-v-${index}`, 'rgba32float', width, height),
      coefficients: createTexture(device, `drc-web-coeff-${index}`, 'rgba32float', width, height),
      coefficientsHorizontal: createTexture(device, `drc-web-coeff-h-${index}`, 'rgba32float', width, height),
    }));
  }
  ...
  public prepare(): void {
    const gain = 2 ** Math.max(0, this.#descriptor.baselineExposure ?? 0);
    const global = generateGlobalToneLut(gain);
    const local = this.#method === '01' ? generateLocalToneLuts(this.#raw, this.#descriptor, global) : null;
    const scalars = new ArrayBuffer(48);
    const view = new DataView(scalars);
    view.setFloat32(0, gain, true);
    view.setFloat32(4, 1, true);
    view.setFloat32(8, 1, true);
    view.setFloat32(12, 1 / 65_536, true);
    view.setFloat32(16, 1 / 256, true);
    view.setFloat32(20, gain * 4, true);
    view.setUint32(24, LEVEL_COUNT, true);
    view.setUint32(28, 1 | (TILES_X << 8) | (TILES_Y << 16), true);
    const cfa = cfaPattern(this.#descriptor.cfa);
    cfa.forEach((channel, index) => view.setFloat32(32 + index * 4, this.#descriptor.whiteBalanceGains[channel] ?? 1, true));
    this.#device.queue.writeBuffer(this.#uniform, 0, scalars);
  ...
  public dispose(): void {
    ...
    for (const level of this.#levels) {
      ...
      level.base.destroy();
      level.statsHorizontal.destroy();
      level.statsVertical.destroy();
  ...
  private encodeGuided(encoder: GPUCommandEncoder, index: number, input: GPUTexture): GPUTexture {
    ...
    this.encodePass(encoder, 'guided_stats_horizontal_main', [[1, input]], 5, resources.statsHorizontal, []);
    this.encodePass(encoder, 'guided_stats_vertical_main', [[1, resources.statsHorizontal]], 5, resources.statsVertical, []);
    this.encodePass(encoder, 'guided_coefficients_main', [[2, resources.statsVertical]], 5, resources.coefficients, []);
...
export function generateGlobalToneLut(gain: number): Float32Array {
  if (!Number.isFinite(gain) || gain <= 0) throw new Error('DRC_TONE_INVALID: gain must be finite and positive');
  const curve = Array.from({ length: LUT_SAMPLES }, (_, index) => {
    const t = index / (LUT_SAMPLES - 1);
    return { x: 2 * (1 - t) * t / gain + t * t, y: 2 * (1 - t) * t + t * t };
  });
...
function generateLocalToneLuts(raw: Uint16Array, descriptor: RawFrameDescriptor, global: Float32Array): Float32Array {
  ...
  for (let y = 0; y < descriptor.height; y += 1) {
    for (let x = 0; x < descriptor.width; x += 1) {
      const channel = cfa[(y & 1) * 2 + (x & 1)] ?? 1;
      const normalized = Math.max(0, Math.min(1, ((raw[y * descriptor.rowStrideSamples + x]! - descriptor.blackLevel) / range) * (descriptor.whiteBalanceGains[channel] ?? 1)));
